...
from backend.models.match import Match, BookmakerOdds
...
class OddsAPIClient:
    ...
    def _parse_bookmaker_odds(self, bookmaker_data: Dict[str, Any]) -> Optional[BookmakerOdds]:
        ...
        try:
            ...
            # Build odds dict by outcome name
            odds_by_name = {o["name"]: o["price"] for o in outcomes}
            
            # We need to map these to home/draw/away later based on match teams
            return BookmakerOdds(
                bookmaker_key=bookmaker_data["key"],
                bookmaker_title=bookmaker_data["title"],
                home_odds=0,  # Will be set later
                draw_odds=None,
                away_odds=0,  # Will be set later
                last_update=datetime.fromisoformat(
                    bookmaker_data["last_update"].replace("Z", "+00:00")
                ),
                # Store raw odds for mapping
            ), odds_by_name
    ...
    async def get_upcoming_matches(
        ...
        Args:
            leagues: List of league keys to fetch (defaults to Config.SUPPORTED_LEAGUES)
            hours_ahead: Only return matches starting within this many hours
            
        Returns:
            List of Match objects with odds from all bookmakers
        """
        if leagues is None:
            ...
            for league in leagues:
                try:
                    ...
                    if response.status_code == 404:
                        # League might not have any matches
                    ...
                    print(f"Fetched {len(matches_data)} matches from {league}")
    ...
    async def get_all_upcoming_odds(self, hours_ahead: int = 48) -> List[Match]:
        """
        Fetch all upcoming matches across all sports (uses 'upcoming' endpoint).
        This is more API-efficient but includes all sports.
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            try:
                ...
                print(f"Fetched {len(matches)} football matches from upcoming endpoint")
